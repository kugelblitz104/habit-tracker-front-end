                    ...
                    <FormProvider {...methods}>
                        <form onSubmit={methods.handleSubmit(onSubmit)}>
                            <Fieldset className='space-y-1'>
                                <TextField isRequired label='Habit name' name='name' placeholder='What will you do?' 
                                    isValid={!errors.name} />                                
                                ...
                                <Controller name='color' control={methods.control}
                                    defaultValue='#aabbcc'
                                    render={({ field }) => <ColorPicker color={field.value} onColorChange={field.onChange}/>}
                                />     
                                <Controller name='frequency' control={methods.control}
                                    defaultValue={{
                                        name: 'daily', frequency: 1, range: 1
                                    }}
                                    render={({field}) => <FrequencyPicker selected={field.value} onSelectedChange={field.onChange} />}
                                />
                                <Controller name='reminder' control={methods.control}
                                    defaultValue={true}
                                    render={({field}) => <LabeledSwitch label='Reminder' checked={field.value} onChange={field.onChange} />}
                                />
                                <Field className='mb-2'>
                                    <Label className='block'>Notes</Label>
                                    <Textarea {...methods.register('notes')} 
